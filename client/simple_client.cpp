...
bool SimpleClient::GDPRDelete(const std::string& user) {
  ...
  /*delete any username data about the user*/
  Delete(user);
  std::string user_copy = user;
  std::string app = "_posts";
  user_copy.append(app);
  /*anonymize remaining posts by deleted user by deleting data
  proving association*/
  Delete(user_copy);
