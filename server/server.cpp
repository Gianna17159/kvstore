...
int KvServer::start() {
  ...
  if (!this->shardcontroller_address.empty()) {
    ...
    //Send a join request to the shardcontroller
...
bool KvServer::process_config() {
  ...
  // Update this->config to reflect the result from querying the
  // shardcontroller
  ...
  //  1. Get all of the server's key-value pairs 

  MultiGetRequest get_req;
  MultiGetResponse get_res;
  get_req.keys = store->AllKeys();
  store->MultiGet(&get_req, &get_res);
  ...
  for (int i=0; i < (int) get_req.keys.size(); i++) {
    std::string curr_key = get_req.keys[i];
    std::string curr_val = get_res.values[i];
    std::optional<std::string> server_exists = this->config.get_server(curr_key);

    if (server_exists) {
      std::string server = *server_exists;
      if (server != address) {
        to_transfer[server][0].push_back(curr_key);
        to_transfer[server][1].push_back(curr_val);

        DeleteRequest del_req;
        del_req.key = curr_key;
        DeleteResponse del_res;
        store->Delete(&del_req, &del_res);
      }
    }
  }
